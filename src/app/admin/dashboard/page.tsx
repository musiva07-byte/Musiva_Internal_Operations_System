    ...
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Common daily tasks for sales, stock, and delivery staff.</p>
          </div>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <ArrowRight aria-hidden className="h-4 w-4 opacity-70 transition-transform group-hover:translate-x-1" />
                </div>
                <p className={action.primary ? "mt-4 font-semibold text-white" : "mt-4 font-semibold text-musiva-plum"}>
                  {action.title}
                </p>
                <p className={action.primary ? "mt-1 text-sm leading-5 text-white/80" : "mt-1 text-sm leading-5 text-muted-foreground"}>
                  {action.helper}
                </p>
              </Link>
            );
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {cards.map((card) => {
          ...
          return (
            <Card key={card.title} className="border-musiva-champagne/60 bg-musiva-porcelain shadow-soft">
            ...
            {attentionItems.map((item) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <ArrowRight aria-hidden className="h-4 w-4 text-muted-foreground transition-transform group-hover:translate-x-1" />
                </div>
              </Link>
            ))}
          </CardContent>
              ...
              {dashboard.recentOrders.length === 0 ? (
                ...
                dashboard.recentOrders.map((order) => (
                  <TableRow key={order.id}>
                    ...
                    <TableCell>{order.customers?.full_name ?? "Customer"}</TableCell>
                    <TableCell>
                      <Badge variant={order.order_status === "cancelled" ? "danger" : "secondary"}>
            ...
            <CardContent className="space-y-3">
              {dashboard.bestSellers.length === 0 ? (
                ...
                dashboard.bestSellers.map((product) => (
                  <div key={product.name} className="flex items-center justify-between gap-3 rounded-md bg-musiva-ivory p-3">
                    <div>
                      <p className="font-medium text-musiva-plum">{product.name}</p>
                      <p className="mt-1 text-xs text-muted-foreground">{product.quantity} units</p>
                    </div>
            ...
            <CardContent className="flex flex-wrap gap-2">
              {dashboard.paymentSummary.length === 0 ? (
                ...
                dashboard.paymentSummary.map((item) => (
                  <Badge key={item.status} variant={item.status === "paid" ? "success" : item.status === "unpaid" ? "danger" : "warning"}>
            ...
            <CardContent className="flex flex-wrap gap-2">
              {dashboard.deliverySummary.length === 0 ? (
                ...
                dashboard.deliverySummary.map((item) => (
                  <Badge key={item.status} variant={item.status === "delivered" ? "success" : item.status === "failed" ? "danger" : "warning"}>
                    {titleize(item.status)}: {item.count}
                  </Badge>
                ))
              )}
