/**
 * Structural regression guard for the Product Catalog CSV export route — same source-text-
 * guard pattern as product-cost-dialog.test.ts. The actual column/permission logic is
 * exhaustively tested in lib/reports/product-catalog-report.test.ts; this file only checks
...
describe("Product Catalog CSV export route", () => {
  ...
  it("uses the required filename convention", () => {
    expect(source).toContain('csvFilename("product-catalog")');
  });
